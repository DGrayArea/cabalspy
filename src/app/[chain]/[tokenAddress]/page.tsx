...
import {
  heliusTokenDataService,
  TokenHolder,
} from "@/services/helius-token-data";
...
function TokenDetailContent() {
  ...
  useEffect(() => {
    ...
    const normalizedChain =
      chain.toLowerCase() === "sol" ? "solana" : (chain.toLowerCase() as any);
  ...
  useEffect(() => {
    if (activeTab === "holders" && holders.length === 0 && !holdersLoading) {
      ...
      heliusTokenDataService
        ...
        .then((s) => {
          if (s) setTotalSupply(s.uiAmount);
        })
        .catch(() => {});
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        const amountRaw =
          tradeType === "buy"
            ? Math.floor(numericAmount * 1e9)
            : Math.floor(numericAmount * Math.pow(10, tokenDecimals || 6));
  ...
  const marketCap =
    ...
    0;
  const volume24h =
    dexData?.volume24h || geckoData?.volume24h || pumpfunData?.volume || 0;
  ...
  const txBuyPct = txns24h
    ? Math.round((txns24h.buys / (txns24h.total || 1)) * 100)
    : null;

  const bondingProgress =
    pumpfunData?.bondingProgress ?? (baseToken as any)?.bondingProgress ?? 0;
  const isMigrated =
    pumpfunData?.isMigrated ||
    (baseToken as any)?.isMigrated ||
    bondingProgress >= 1;
  ...
  const platform = aiPlatformDetector.detectPlatform({
    ...
    chain,
    raydiumPool: pumpfunData?.raydiumPool || (baseToken as any)?.raydiumPool,
  });
  ...
  const socialLinks = useMemo(() => {
    ...
    const twitter =
      dexData?.socials?.find((s) => s.type === "twitter")?.url || null;
    const telegram =
      dexData?.socials?.find((s) => s.type === "telegram")?.url || null;
      ...
      <main className="relative z-10 w-full max-w-screen-2xl mx-auto px-2 sm:px-4 pt-2 sm:pt-6 pb-20">
        ...
        <section className="glass rounded-2xl sm:rounded-3xl p-3 sm:p-4 border border-white/10 mb-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <button
                ...
                className={`p-1.5 sm:p-2 rounded-xl border cursor-pointer transition-all ${isWatchlisted ? "bg-primary/20 border-primary/50 text-primary" : "bg-white/5 border-white/10 text-muted hover:text-white hover:border-white/30"}`}
              >
                <Star
                  className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isWatchlisted ? "fill-current" : ""}`}
                />
              </button>
              <div
                className="flex items-center gap-1 cursor-pointer px-2 py-1.5 rounded-lg bg-white/5 border border-white/10 hover:border-white/20 transition-all"
                onClick={copyAddress}
              >
                <code className="text-[8px] sm:text-[9px] font-mono text-muted hidden xs:block">
          ...
          <div className="flex items-center gap-3 sm:gap-6 mt-2.5 pt-2.5 border-t border-white/5 flex-wrap">
            ...
            {txBuyPct !== null && (
              <div className="flex flex-col gap-1 min-w-[64px] sm:min-w-[80px]">
                ...
                <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-linear-to-r from-primary to-primary/70 rounded-full transition-all"
                    style={{ width: `${txBuyPct}%` }}
                  />
                </div>
              </div>
            )}
            ...
            {/* Address — only on sm+ */}
            <div
              className="hidden sm:flex items-center gap-1.5 cursor-pointer ml-auto px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 hover:border-white/20 transition-all"
              onClick={copyAddress}
            >
            ...
            <section className="glass rounded-2xl sm:rounded-3xl border border-white/10 overflow-hidden">
              <div className="flex border-b border-white/10 bg-black/40">
                {[
                  ...
                  { id: "cabalspy", label: "🔍 CABALSPY INTEL" },
                ].map((tab) => (
                  <button
                    key={tab.id}
                    onClick={() => {
                ...
                {chartTab === "chart" ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 text-center px-6">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted uppercase tracking-widest">
                        CabalSpy Intel doesn&apos;t track native {tokenSymbol}
                      </p>
                    </div>
                  </div>
                ) : (
                    ...
                    {!isAuthenticated ? (
                      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-panel/60 backdrop-blur-md rounded-2xl text-center p-6">
                        ...
                        <p className="text-[10px] text-muted mb-4">
                          Sign in to access live trade data
                        </p>
                        <AuthButton />
                      </div>
                    ) : (
                            ...
                            [
                              {
                                label: "5 MIN",
                                txns: dexData?.txns5m,
                                vol: dexData?.volume5m,
                              },
                              {
                                label: "1 HOUR",
                                txns: dexData?.txns1h,
                                vol: dexData?.volume1h,
                              },
                              {
                                label: "6 HOURS",
                                txns: dexData?.txns6h,
                                vol: dexData?.volume6h,
                              },
                              {
                                label: "24 HOURS",
                                txns: dexData?.txns24h,
                                vol: dexData?.volume24h,
                              },
                            ] as const
                          )
                            .filter((r) => r.txns)
                    ...
                    {holdersLoading ? (
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <div className="text-[10px] font-mono text-white font-bold">
                                  {heliusTokenDataService.formatAddress(
                                    h.address,
                                  )}
                                </div>
                                <div className="text-[8px] text-muted uppercase">
                                  {formatNumber(h.uiAmount)} tokens
                                </div>
                              </div>
                            </div>
                      ...
                      <div className="py-8 text-center space-y-3">
                        ...
                        <p className="text-[9px] text-muted/60">
                          View on Bubblemaps or Solscan for full distribution
                        </p>
                        ...
                        {[
                          {
                            ...
                            value: totalSupply
                              ...
                              : pumpfunData?.totalSupply
                                ? formatNumber(pumpfunData.totalSupply)
                                : "1,000,000,000",
                          },
                          ...
                          { label: "Network", value: chain.toUpperCase() },
                          {
                            ...
                            value: isMigrated
                              ? "GRADUATED ✓"
                              : `BONDING ${(bondingProgress * 100).toFixed(0)}%`,
                          },
                          {
                            label: "DEX",
                            value: dexData?.dexId
                              ? dexData.dexId.charAt(0).toUpperCase() +
                                dexData.dexId.slice(1)
                              : platformName || "—",
                          },
                          {
                            label: "24H TXS",
                            value: txns24h
                              ? `${formatNumber(txns24h.buys)}B / ${formatNumber(txns24h.sells)}S`
                              : "—",
                          },
                        ].map(({ label, value }) => (
                          <div
                  ...
                  >
                    ...
                    <span className="relative z-10 flex items-center justify-center gap-2">
                      {isTrading ? (
                        ...
                        <>
                          <Zap
                            className={`w-4 h-4 ${tradeType === "buy" ? "fill-black" : "fill-white"}`}
                          />
                          {tradeType === "buy"
                            ? "CONFIRM PURCHASE"
                            : "CONFIRM SALE"}
                    ...
                    <div className="rounded-3xl border border-white/10 bg-white/5 p-4 text-sm space-y-3">
                      ...
                      <div className="space-y-3">
                        {tradeHistory.map((entry) => (
                          <div
                            key={entry.id}
                            className="rounded-2xl bg-black/20 p-3 border border-white/5"
                          >
                            <div className="flex items-center justify-between text-[9px] uppercase tracking-[0.2em] text-muted mb-2">
                            ...
                            <div className="flex items-center justify-between text-[10px] text-gray-300 mt-2">
                              ...
                              {entry.signature ? (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="text-[10px] text-primary hover:text-white"
                                >
                                  View
                                </a>
                              ) : null}
                        ...
                        <div className="text-right">
                          ...
                          {txns && (
                            <div className="text-[8px] text-muted">
                              <span className="text-primary">{txns.buys}B</span>{" "}
                              /{" "}
                              <span className="text-accent">{txns.sells}S</span>
                            </div>
                          )}
                        </div>
                      </div>
