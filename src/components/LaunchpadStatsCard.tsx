...
export default function LaunchpadStatsCard() {
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-1">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="w-10 h-8 bg-gray-800/30 rounded-md"
                ></div>
              ))}
            </div>
          </div>
  ...
  const filteredLaunchpads = stats.launchpads
    ...
    .sort((a, b) => {
      const aStats = a[
        `stats${selectedTimeframe}` as keyof LaunchpadStats
      ] as LaunchpadStats["stats1d"];
      const bStats = b[
        `stats${selectedTimeframe}` as keyof LaunchpadStats
      ] as LaunchpadStats["stats1d"];
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Performance metrics across top platforms
          </p>
        </div>
          ...
          >
            <div className="flex gap-4 min-w-max">
              {filteredLaunchpads.map((lp, index) => {
                ...
                const displayName =
                  launchpadDisplayNames[lp.launchpad] || lp.launchpad;
                  ...
                  >
                    ...
                    <div className="space-y-2 relative z-10">
                      ...
                      <div className="flex items-center justify-between px-3 py-2 bg-gradient-to-r from-purple-500/10 to-purple-500/5 rounded-lg border border-purple-500/20 group-hover:border-purple-500/40 transition-all">
                        ...
                        <span className="text-xs font-bold text-purple-300 truncate ml-1">
